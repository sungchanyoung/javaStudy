...
public class KeywordExtractionAnalysis implements TextAnalysisStrategy{
    ...
    public AnalysisResult analyze(String text) {
        ...
        sortedWordScores.sort(Map.Entry.<String, Double>comparingByValue().reversed());
        ...
        for (Map.Entry<String, Double> entry : sortedWordScores) {
            ...
            if (!topKeywordScores.containsKey(keyword) || topKeywordScores.get(keyword) < score) {
                count++;
            }
        }
        result.addResultData("분석된 총 단어 수", words.size());
        result.addResultData("불용어 제외 고유 단어 수", wordFrequency.size());
        result.addResultData("상위 키워드", topKeywords);
        result.addResultData("키워드 점수", topKeywordScores);
