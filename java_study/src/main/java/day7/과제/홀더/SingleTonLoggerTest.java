...
public class SingleTonLoggerTest {
    public static void main(String[] args) throws Exception {
        // 첫 번째 로거 인스턴스 가져오기
        HolderSingletonLogger logger1 = HolderSingletonLogger.getInstance();
        logger1.initialize("application_singleton.log", HolderSingletonLogger.LogLevel.INFO);

        // 두 번째 로거 인스턴스 가져오기
        HolderSingletonLogger logger2 = HolderSingletonLogger.getInstance();
        ...
        // 로그 레벨 변경 후 테스트
        System.out.println("\n로그 레벨 변경 테스트:");
        logger1.setMinLogLevel(HolderSingletonLogger.LogLevel.DEBUG);
        ...
        for (int i = 0; i < 5; i++) {
            new Thread(() -> {
                HolderSingletonLogger threadLogger = HolderSingletonLogger.getInstance();
